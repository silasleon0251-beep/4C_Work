...
func init_slider(slider: HSlider, volume_type: VolumeType, title_text: String, is_reset:bool = false):
	if not slider:
		print("警告：未配置 ", title_text, " 滑块")
		return
		
	# 1. 设置滑块基础属性
	var title_label = slider.get_parent().get_node_or_null("TitlePanel/Title") 
	var num_label = slider.get_parent().get_node_or_null("Panel/Number")   
	var test_btn = slider.get_parent().get_node_or_null("TestButton")  
	...
	if not is_reset:
		...
		slider.value_changed.connect(func(value): _on_slider_value_changed(slider, volume_type, value))
...
func _on_slider_value_changed(slider: HSlider, volume_type: VolumeType, value: float):
...
func _on_test_pressed(volume_type: VolumeType) -> void:
	# 1. 获取对应类型的音量值
	var volume_value = _get_volume_value_by_type(volume_type)
	# 2. 获取对应类型的音频播放器
	var target_player = _get_audio_player_by_type(volume_type)
	# 3. 设置音量并播放
	_set_volume_and_play(target_player, volume_value)
	
	print("播放测试音效: ", volume_type)
...
## 根据音量类型获取对应的音频播放器
## @param volume_type 音量类型（VolumeType枚举）
## @return AudioStreamPlayer2D 对应类型的音频播放器节点
## 功能：根据传入的音量类型，返回预绑定的测试音频播放器节点
func _get_audio_player_by_type(volume_type: VolumeType) -> AudioStreamPlayer2D:
	match volume_type:
		VolumeType.main:
			return main_audio_test
		VolumeType.bgm:
			return bgm_audio_test
		VolumeType.sound:
			return eff_audio_test
	return null  # 兜底默认值

## 将 0~100 的线性滑块值 → 转成人耳等比的音量值(0~100)
## @param linear_val 线性音量值（0~100）
## @return int 对数转换后的音量值（0~100）
## 功能：
## 1. 限制输入值在0~100范围内
## 2. 0值直接返回0（静音）
## 3. 非0值通过对数曲线转换，模拟人耳听觉的均匀变化
...
## 计算最终实际音量（主音量×子音量）
## @param master 主音量(0~100)
## @param vol BGM 或 音效自身音量(0~100)
## @return int 最终实际音量(0~100)
## 功能：
## 1. 将主音量和子音量分别转换为对数音量
## 2. 计算两者的乘积比例（主音量权重×子音量）
## 3. 限制结果在0~100范围内
func get_final_volume(master: int, vol: int) -> int:
	var master_log = get_log_volume(master)
	var vol_log = get_log_volume(vol)
	var final = (master_log * vol_log) / 100.0  # 总控比例
	return clamp(final, 0, 100) as int

## 通用的音量设置和播放逻辑
## @param player 目标音频播放器节点
## @param volume_value 音量值（0~100）
## @return void
## 功能：
## 1. 根据主音量和当前类型音量计算最终音量
## 2. 音量为0时设置为-80dB（静音），否则设置为计算出的dB值
## 3. 播放音频并打印播放器名称
func _set_volume_and_play(player: AudioStreamPlayer2D, volume_value: float) -> void:
	
	# 统一的音量计算逻辑
	if volume_value != 0:
		player.volume_db = get_final_volume(GlobalConfig.main_volume_value, int(volume_value))
	else:
		player.volume_db = -80
	# 播放音频
	player.play()
...
##    - 启用BGM/音效滑块编辑
##    - 重置临时音量保存标记
func _sync_volume_with_main():
	# 获取当前主音量值
	var main_volume = GlobalConfig.main_volume_value
	...
	if main_volume == 0:
		...
		if bgm_volume_slider:
			...
			var bgm_num_label = bgm_volume_slider.get_parent().get_node_or_null("Panel/Number")
			if bgm_num_label:
				bgm_num_label.text = "0"
		
		if sound_volume_slider:
			sound_volume_slider.value = 0
			var sound_num_label = sound_volume_slider.get_parent().get_node_or_null("Panel/Number")
			if sound_num_label:
